...
exports.plugin = {
    ...
    async register (container, options) {
        ...
        container.resolvePlugin('p2p').getMonitor().broadcastTransactions = async transactions => {
            ...
            const transactionsBroadcast = transactions.unfiltered.map(tx => tx.toJson())

            return Promise.all(peers.map(peer => p2p.communicator.postTransactions(peer, transactionsBroadcast)))
        }
    }
}
